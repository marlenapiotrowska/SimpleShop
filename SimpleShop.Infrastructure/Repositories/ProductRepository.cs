...
internal class ProductRepository : IProductRepository
{
    ...
    {
        var productDb = await GetByIdBaseAsync(product.Id);
    ...
    {
        var product = await GetByIdBaseAsync(id);
    ...
    {
        var product = await GetByIdBaseAsync(productId);

        return _factory.Create(product);
    }

    private async Task<ProductDb> GetByIdBaseAsync(Guid productId)
    {
